# File for handling deprecation of pymoo.factory.get_mutation
import numpy as np
...
class IntegerFromFloatCrossover(Crossover):
    ...
    def __init__(self, clazz=None, **kwargs):
        ...
        super().__init__(self.crossover.n_parents, self.crossover.n_offsprings, prob=self.crossover.prob.value)
...
def get_mutation(name, eta, prob):
    if name == 'int_pm':
        return IntegerFromFloatMutation(clazz=PolynomialMutation, eta=eta, prob=prob)
    else:
        raise RuntimeError


def get_crossover(name, eta, prob):
    if name == 'int_sbx':
        return IntegerFromFloatCrossover(clazz=SimulatedBinaryCrossover, eta=eta, prob=prob)
    else:
        raise RuntimeError