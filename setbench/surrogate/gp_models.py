...
from setbench.surrogate.base_surrogate import BaseSurrogate
from setbench.tools import transform_op as gfp_transforms
from setbench.tools.metric_op import quantile_calibration

from setbench.tools.gp_op import fit_gp_surrogate
from gpytorch.priors.lkj_prior import LKJPrior
...
class MultiTaskExactGP(BaseGPSurrogate, KroneckerMultiTaskGP):
    def __init__(self, feature_dim, out_dim, encoder, likelihood=None, covar_module=None,
        ...
        # initialize common attributes
        BaseGPSurrogate.__init__(self, encoder=encoder, *args, **kwargs)
        # if 'task_covar_prior' in kwargs and kwargs['task_covar_prior'].correlation_prior \
        #         and isinstance(kwargs['task_covar_prior'].correlation_prior, LKJPrior):
        #     kwargs['task_covar_prior'].correlation_prior.concentration = kwargs['task_covar_prior'].correlation_prior.concentration.to(self.device, self.dtype)
