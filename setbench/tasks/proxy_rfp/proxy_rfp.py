...
from setbench.tools.string_op import FoldedCandidate
...
class ProxyRFPTask(BaseTask):
    def __init__(self, tokenizer, candidate_pool, obj_dim, max_len, transform=lambda x: x,
                 num_start_examples=1024, task_name='', **kwargs):
    ...
    def task_setup(self, config, project_root=None, *args, **kwargs):
        ...
        rfp_known_structures = pd.read_csv(
            f'{project_root}/setbench/assets/fpbase/rfp_known_structures.csv'
        )
        ...
        seed_data = pd.read_csv(
            f'{project_root}/setbench/assets/fpbase/proxy_rfp_seed_data.csv'
        )
        ...
        for row_idx, datum in rfp_known_structures.iterrows():
            ...
            parent_pdb_path = f'{project_root}/setbench/assets/foldx/{pdb_id}_{chain_id}/wt_input_Repair.pdb'
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for query_pt in x:
            ...
            for i in range(self.max_num_edits):
                ...
                assert op_idx == 0, 'FoldX only accepts substitutions'
                mut_res = self.tokenizer.sampling_vocab[mut_res_idx]
                mut_seq = apply_mutation(mut_seq, mut_pos, mut_res, op_type, self.tokenizer)
            candidate = self.make_new_candidates(np.array([base_candidate]), np.array([mut_seq]))[0]
