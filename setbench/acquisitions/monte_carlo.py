...
class qDiscreteUCBHVI(qExpectedHypervolumeImprovement):
    def __init__(self, beta=0.1, **kwargs):
        super().__init__(**kwargs)
        self.beta = beta
        print("beta is set to ", beta, " in qDiscreteUCBHVI")
    ...
    def forward(self, X: array) -> Tensor:
        ...
        ucb = posterior.mean + self.beta * torch.sqrt(posterior.variance)
        hvi = self._compute_qehvi(samples=ucb.unsqueeze(0)) # unsqueeze for samples dimension.
        return hvi
