...
def setup(config):
    ...
    wandb.init(project='setbench_multi', config=log_config, mode=config.wandb_mode,
               group=config.exp_name)
    config['job_name'] = wandb.run.name
    config, _ = startup(config)  # random seed is fixed here

    torch.backends.cudnn.benchmark = False # this is set as False in startup to reproducibility
    torch.backends.cudnn.deterministic = True # this is set as True in startup to reproducibility

    return config

@hydra.main(config_path='../hydra_config', config_name='active')
def main(config):
    # setup wandb and random seed
    config = setup(config)
        ...
        try:
            ...
            with max_cholesky_size(max_chol_sz):
                ...
                metrics = optimizer.optimize(
                    base_candidates, base_targets, all_seqs, all_targets, log_prefix=config.task.log_prefix, seed=config.seed
                )
            ...
            ret_val = float('NaN')
            wandb.finish(exit_code=1)  # necessary to log Hydra multirun output to different jobs
            return ret_val
